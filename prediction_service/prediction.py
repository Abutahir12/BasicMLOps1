...
schema_path = os.path.join("prediction_service", "wine_schema.json")
...
def validate_input(dict_request):
    ...
    def _validate_values(col, val):
        ...
        if (schema[col]["min"] <= float(dict_request[col]) <= schema[col]["max"]):
            pass
        else:
            raise NotInRange
...
def form_response(dict_request):
    if validate_input(dict_request)==True:
